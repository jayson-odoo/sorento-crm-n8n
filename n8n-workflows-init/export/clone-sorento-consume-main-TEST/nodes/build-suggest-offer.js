...
const _isDerivedQueryToken = (tok) => {
  ...
  if (!k) return false;
  if (_sentTokens.size) {
    if (_sentTokens.has(k)) return false;
    // resolve-entity strips qualifier prefixes ("Customer Mastiles Klang" -> token
    // "Mastiles Klang"), so exact membership misses a genuinely-sent token and this
    // guard then eats the REAL miss from the DYM (measured: execs 13184999/13186532 —
    // the customer's MASTILE KLANG alternatives never rendered). A token contained
    // inside ONE parser raw was sent by us; the CRM's query-keyed resolution (the
    // whole sentence) can never be a substring of a single entity raw.
    if (k.length >= 3) { for (const s of _sentTokens) { if (s.includes(k)) return false; } }
    return true;
  }
  return !!_rawTurn && k === _rawTurn;
};
// A token whose matches SURVIVED to compatible_entities was not missed — it passed
// (an ambiguous product's variants all get queried on the answer lane; the exact same
// matches sit in the gate output). Without this, a gated turn re-labels the passed
// product a miss and asks about the wrong token ("Couldn't find Srtwc286" while the
// customer was the miss — measured, exec 13184999).
const _compatKeys = (() => {
  try {
