...
// ── CUSTOMER PICKER ROSTER SURVIVES THE ANSWER (captain, 2026-08-20) ─────────
...
// or on a domain change.
{
  ...
  const _cpBornNow = !!(_cpGate && _cpGate.require_specific === true
    ...
    && _cpGate.compatible_entities.every(c => String(c && c.entity_type).toLowerCase() === 'customer')
  ...
  const _cpCarried = (!_cpBornNow
    && Array.isArray(_cpPrev.cust_last_result_set) && _cpPrev.cust_last_result_set.length > 0
    && _cpPrev.cust_pick_domain === qf.domain_hint
    && !_cpFreshTyped)
      ? _cpPrev.cust_last_result_set : null;
  const _cpSet = _cpBornNow ? last_result_set : _cpCarried;
  if (_cpSet) {
    output.variables.cust_last_result_set = _cpSet;
    output.variables.cust_pick_domain     = _cpBornNow ? (qf.domain_hint ?? null) : _cpPrev.cust_pick_domain;
