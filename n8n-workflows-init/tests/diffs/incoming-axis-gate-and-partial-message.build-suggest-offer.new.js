...
if (!anyUuidAlt) {
  ...
  if (axis === 'date') {
    ...
    // §21.8: incoming empty-AND (container(s) + a resolved product NOT in that container → get-results
    // empty → suggest-on-miss). Name the product + container(s) instead of the fallen-through first
    // entity raw (WHSU's compatible_entity has code:null, so askedCode wrongly became "BMOU…"). TEXT ONLY —
    // values / suggest_quick_reply / suggest_last_result_set are unchanged.
    const containerRaws = (Array.isArray(q?.entities) ? q.entities : []).filter(e => e.hint === 'inbound_shipment').map(e => e.raw).filter(Boolean);
    const productRaws   = (Array.isArray(q?.entities) ? q.entities : []).filter(e => e.hint === 'product').map(e => e.raw).filter(Boolean);
    if (q?.domain_hint === 'incoming' && containerRaws.length && productRaws.length) {
      text =
        `No incoming stock of ${productRaws.join(', ')} in ${containerRaws.join(', ')}. Try: ${values.join(', ')}. ` +
        `Reply with a code to continue, or would you like me to escalate to ${team} team?`;
    } else {
      text =
