...
// Mirrors the friendly-disclaimer IIFE above (same happy-path guards, same append-to-userResponse).
// On the found-with-partial incoming/order path, itemize the axis tokens the gate could not
// resolve (gate.not_found_axis_tokens) so a container/order that resolved AND one that didn't
// are BOTH acknowledged. Product-axis partials (PRODUCT MODE) itemize gate.not_found_product_tokens.
(() => {
  ...
  const noun = NOUN[qf.domain_hint];   // identifier-axis noun (undefined for product-axis domains)
  const nrm = t => String(t).toLowerCase().trim();

  // identifier-axis bucket (incoming/order IDENTIFIER MODE) — needs a domain noun
  const axisTokens = (noun && Array.isArray(gate.not_found_axis_tokens)) ? [...gate.not_found_axis_tokens] : [];
  const axisSet = new Set(axisTokens.map(nrm));
  // product-axis bucket (PRODUCT MODE proceed path, any product-axis domain) — noun is always 'product',
  // independent of the domain NOUN map; dedupe against axis tokens.
  const productTokens = (Array.isArray(gate.not_found_product_tokens) ? gate.not_found_product_tokens : [])
    .filter(t => !axisSet.has(nrm(t)));

  if (axisTokens.length) {
    userResponse += `\n\nI couldn't find any ${noun} matching:\n` + axisTokens.map(t => `• ${t}`).join('\n');
  }
  if (productTokens.length) {
    userResponse += `\n\nI couldn't find any product matching:\n` + productTokens.map(t => `• ${t}`).join('\n');
  }
})();
