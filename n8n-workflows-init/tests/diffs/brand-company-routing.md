...
| `Call 'sub-human-intervention'` (`133fcc06-…`) §3.7 | `workflowInputs.value.brand_code = ={{ $('escalation-context').isExecuted ? ($('escalation-context').first().json.brand_code \|\| '') : '' }}`, `.company_id` likewise (rev-8: `.first()` on an existing-but-unexecuted node THROWS before `\|\| ''` can apply, so the guard is required on any inbound edge that bypasses `escalation-context`); `workflowInputs.schema` += `{id:'brand_code',type:'string',removed:false}`, `{id:'company_id',…}` (12 existing entries unchanged). Still targets the fork `vUfFUDjLAuMaeQE6`, still `is_test: true`. | `setNodeParameter /workflowInputs/value/brand_code`, `…/company_id`, `…/schema` |
...
### 1g. rev-8 patch (sixth review round, captain-decided) — `Call 'sub-human-intervention'` params + `compile-current-state`

| # | artifact | change | why |
|---|---|---|---|
| 1 | `spine-get-cs-members+call-hi.after.jsonl` (HI call `workflowInputs.value`) | `brand_code` / `company_id` → `={{ $('escalation-context').isExecuted ? ($('escalation-context').first().json.<key> \|\| '') : '' }}` | `$('node').first()` on an existing-but-UNEXECUTED node throws in n8n — `\|\| ''` never gets to run — so the rev-1 form was only safe while `escalation-context` owned the single inbound edge of the HI call. True on the clone (one rewire, B8 confirms), NOT verified on live `9qVyfUxmRQqrpGRMDLRuz` (`726da5dc-…`), which diverges from the clone in both directions. An escalation arriving on any other inbound edge would throw instead of assigning, on real customers. Every other new cross-node reference in this change set is already guarded (`$('cs-roster-plan')` in try/catch); this one now is too |
| 2 | `spine-compile-current-state.js` (Δ4 merge arm) | multi-company rendering keys off `_mem.cs_multi_note` (companies QUERIED, the same signal `build-cs-member-offer` uses) instead of the count of companies that contributed a member; the `[ X: no customer-service members are configured — omitted. ]` lines are appended from `_mem.routing_companies` for any queried company absent from the shown membership set | the two arms disagreed exactly in the B5b shape — a two-company plan whose second roster came back empty. The plain path named both companies and disclosed the omission; the merged path rendered an unlabelled single-company list with neither. Rev-7's stated goal was that the arms cannot drift; now the same signal drives both. Single-company output unchanged (no note ⇒ no labels, no omission lines) |

**The pending clone republish (P6) must include the HI-call param change**, not just the Code bodies — it is a
`workflowInputs.value` edit on `Call 'sub-human-intervention'` (`133fcc06-…`), and P3 gains an explicit live-topology
step (enumerate every inbound edge of live `726da5dc-…` before rewiring).
