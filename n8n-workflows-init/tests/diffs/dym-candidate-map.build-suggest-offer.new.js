...
// PHASE-2 SEAM: multi-product family union (family-loop) not built — single base only.
{
  ...
  if (sibXfRan && sibProbeRan && domIncoming) {
    ...
    if (Array.isArray(fam) && fam.length) {
      ...
      if (extras.length > 0) {
        // Does ANY sibling actually have incoming? (same has/no flag derived from sibling-probe)
        const anyHasIncoming = sibs.some(s => s.has);
        ...
        if (anyHasIncoming) {
          // ── COMBINED PICKER (≥1 sibling has incoming) — UNCHANGED behaviour ──
        ...
        // ── NO sibling has incoming → LIST-ONLY + ESCALATE-ONLY variant ──
        // Still list every sibling ("— no incoming") for information, but DROP the numbered
        // pick entirely: no number-pick invite and NO positional affordance. Keep the SAME
        // suggest_offer envelope so the yes→escalate / no→decline round-trip still reconciles
        // via the fork's suggest-follow-up (is_affirmative branch). last_result_set is EMPTY so a
        // stray typed number maps to nothing (output_exchange → out-of-range → no sibling re-query),
        // making the message ("would you like me to escalate?") and the behaviour agree.
        const listed = sibs.map(s => `${s.code} — no incoming`).join('\n');
        out.suggest_offer = true;
        out.suggest_selection_context = 'suggest_offer';
        out.suggest_response =
          `No incoming stock (ETA) found for ${exactList}. Related products:\n` +
        ...
        // Yes/No escalate buttons ONLY — no numbered options (comma-stripped so labels don't split).
        out.suggest_quick_reply = [YES, NO].map(s => String(s).replace(/,/g, '')).join(',');
        // EMPTY: no positional pick — a bare number must NOT re-query a sibling.
