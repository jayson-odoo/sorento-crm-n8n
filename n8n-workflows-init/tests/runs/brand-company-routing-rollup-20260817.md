...
`company_ids` MEMBERSHIP set, so shared CS staff no longer make a company look empty in the reply or vanish from the persisted plan. **Rev-4 also CHANGED the B4/B5 expectations** — the bare-"yes" arm now sends the persisted `routing_roster_plan` pair
...
**Headline: §0 S1–S6 ZERO-EGRESS HELD on all 17 clone executions. No HALT.** Functional: B1 B2 B3 B7 B8 B9 P1 PASS;
