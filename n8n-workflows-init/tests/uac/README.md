...
| `TA.md` | §TA ×25 | — | **tier-only access ask** — brand×tier split, numbered multi-select tier ask (fires only when >1 tier entitled + none stated), always-attach answers, compat mapper. §TA-7 is the non-persistence gate; §TA-11 the brand-gate fail-closed; §TA-14 the roster-collision guard; §TA-16..18 the brand denial surviving the turn (F6/F7 — a denial outranks the ask, and brand scope carries with the query scope). Plan: `plans/access-tier-ask-plan.md` |
| `SA.md` | §SA ×6 + §SA-P ×5 | — | **spec-search shape A wiring** — descriptive product phrases resolve via the CRM spec ranker (`spec_fallback` + deterministic `free_terms`, one-node body edit on `resolve-entity`). §SA-P1..5 are offline contract probes vs the LIVE CRM; §SA-1 needs its RED on the pre-change rev; §SA-2 is the code-query byte-parity gate; §SA-4 bans the unscoped universal negative. Plan: `plans/spec-search-shapeA-wiring-plan.md` |
| `HT.md` | §HT ×18 + §HT-FP ×4 | — | **human-intervened timeout** — 30s sweeper clears the flag after X min human silence + intervene/timeout notices; kill-switch + pilot allowlist; §HT-9 is the ghost-message gate. Plan: `plans/human-intervened-timeout-plan.md` |
| `SR.md` | §SR-U ×5 + §SR ×8 | — | **spec-answer honesty (SR-1)** — N-0 send the customer's qualifiers in `free_terms` (joined, adjacency preserved), N-2 say what was ignored, N-3 outcome-keyed suppression of "Couldn't find these" under a spec answer. §SR-2 is the 1.0mm-vs-1.2mm RED; §SR-4 guards over-suppression; §SR-8 documents the OR-path brand gap (CRM C-1). Plan: `plans/spec-answer-honesty-plan.md` |
| `SB.md` | §SB-P ×9 + §SB-R ×6 + §SB ×9 | — | **shape B `require` predicate** — ⛔ gated on CRM deploy (built `5ded454b8`) + SA landing. §SB-P8 pins the 422 unknown-key guard; §SB-4 unrecognized→clarify never "none"; §SB-5 the only allowed unscoped negative; §SB-8 re-runs SA-2 parity. Plan: `plans/spec-search-shapeB-predicate-plan.md` |
