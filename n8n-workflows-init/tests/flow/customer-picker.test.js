...
test('flow: customer-picker (disallowed-entity-gate -> cs-offer-gate FALSE -> compile-current-state)', async (t) => {
  ...
  await t.test('the customer-facing reply carries the annotation, with a plain hyphen', () => {
    ...
    // window and the miss suffix must stay BOUNDED - a bare "- no delivery" would be a false
    // universal (the probe never looked further back than 90 days). "recent" claims exactly what
    // was measured without naming a window that reads like a search limit (captain copy decision
    // 2026-08-25: window wording dropped, 'recent' keeps the claim bounded). The dated-turn arm,
    // where the plain suffix IS the honest one, is pinned by the unit fixture
    // hand-annotate-customer-dated-window.json.
    assert.ok(reply.includes('2. SB SMART CONCEPT SDN BHD - no recent delivery'), 'the unprobed-negative company carries the bounded miss suffix: ' + JSON.stringify(reply));
