...
// ── N-1a · say WHAT matched, on spec answers only (spec-answer-honesty SR-1b) ────────
...
// side and a right answer look identical to them. This is the smallest honest statement the
// wire can support today: the union of `display.matched_specs` over the spec rows that are
// IN THEIR ANSWER, deduped, in first-seen order, humanised.
//
// KEY NAMES ONLY. `matched_specs` is a list of registry keys (references.py
// `_emit_spec_matches` <- product_spec_search.py `sorted(set(matched))`); the VALUES are not
// on the wire in any form until CRM C-3 lands. Rendering "double bowl" from `bowl_count`
// would be inventing the very data this change exists to stop inventing.
//
// `free_terms` is NEVER rendered as a key: it is the machinery that says "your free text hit
// the rendered spec sentence", not a spec anyone can name. When it is the ONLY thing that
// matched, the honest statement is that we matched on the description itself.
//
// Rows come from the two places `_emit_spec_matches` writes (`resolutions[].matches`, and
// `intersection` on the AND branch) and are joined to the list the customer was SHOWN, the
// same join N-3 uses. No in-answer spec row contributes a key => no line, byte-identical.
(() => {
  ...
  if (!_shown.size) return;
  const _rows = [];
  if (Array.isArray(_specRes.resolutions)) {
    for (const _res of _specRes.resolutions) if (Array.isArray(_res && _res.matches)) _rows.push(..._res.matches);
  }
  if (Array.isArray(_specRes.intersection)) _rows.push(..._specRes.intersection);
  const _inAnswer = (m) => [m && m.uuid, m && m.canonical_code]
    .some(v => { const k = String(v ?? '').trim().toLowerCase(); return k && _shown.has(k); });
  // First-seen order across rows, deduped. Row order is the ranker's order, so the key that
  // did the most work on the best-ranked product is named first.
  const _keys = [];
  for (const m of _rows) {
    if (String(m?.match_tier ?? '').toLowerCase() !== 'spec_search') continue;
    if (!_inAnswer(m)) continue;
  ...
  // THE NO-OP GUARANTEE, and the ONLY gate: no spec key in the answer => no line, and the
  // whole output stays byte-identical to the pre-SR-1b body. Deliberately not written as a
  // separate "was there a spec row" flag - a second guard for the same outcome would be
  // unfalsifiable (LESSONS 66: a mutant that cannot go red is a lie about the assertion set).
  if (!_keys.length) return;
  // Machinery, not specs. THE one place an exclusion goes - see the house-preference finding
  // in ccs-hunks.js (`matched_specs` can name a key the customer never stated).
  const _MACHINERY_KEYS = ['free_terms'];
  const _real = _keys.filter(k => _MACHINERY_KEYS.indexOf(k.toLowerCase()) < 0);
  ...
  // When the CRM ships spec VALUES on the result rows, the ONLY line that changes is the
  // one below: map each key to "${key}: ${value}" from the row that carried it, keeping
  // this block's shape - the union/dedup/first-seen order, the free_terms exclusion, the
  // description fallback and the no-op guarantee all stay exactly as they are.
  userResponse += _real.length
    ? `\n\n_Matched on: ${_and(_real.map(_pretty))}._`
...
(() => {
  ...
  const _rawMsg = (() => { try { const _j = $('tf-message').first().json;
...
(() => {
  ...
  // descriptive words were answered. Scoped by the SAME code-shape rule the shipped
  // `free_terms` expression uses, so only tokens that could have fed spec search are covered
  // — an unknown CODE beside a spec answer is excluded from free_terms and still surfaces
  // its own miss (UAC SR-U5). Zero spec_search rows in the answer => this is inert.
  const _fedFreeTerms = (_e) => (Array.isArray(_e) ? _e : []).map(x => String((x && x.raw) || '').trim()).filter(v => v.length > 0 && !(/[0-9]/.test(v) && /^[A-Za-z][A-Za-z][A-Za-z0-9._\/\-\u2010-\u2015\u2212\uFE58\uFE63\uFF0D]*$/.test(v))).filter((v, i, a) => a.indexOf(v) === i);
