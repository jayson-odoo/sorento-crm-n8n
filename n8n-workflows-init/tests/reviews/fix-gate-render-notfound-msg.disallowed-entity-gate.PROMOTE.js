...
if (REQUIRE_SPECIFIC_DOMAINS.has(domain)) {
  ...
  if (orResolutions) {
    // ── OR-MODE: exact = match_tier 'exact' ──
    const pickExact = (r) => {
      const exacts = (r.matches ?? []).filter(m => m && m.match_tier === 'exact' && isCompatible(m));
    ...
    for (const r of orResolutions) {
      ...
      const isProductToken = matches.every(m => String(m.entity_type).toLowerCase() === 'product');
    
      if (isProductToken) {
        // PRODUCT: exact-tier wins (user typed a full code; ignore -BL/-150 prefix variants).
        const exacts = matches.filter(m => m.match_tier === 'exact');
        if (exacts.length === 1) {
          const m = exacts[0];
          exact_entities.push({ uuid: m.uuid, entity_type: m.entity_type, code: m.canonical_code });
        } else if (matches.length === 1) {
          // single product match, no exact → take it
          const m = matches[0];
          exact_entities.push({ uuid: m.uuid, entity_type: m.entity_type, code: m.canonical_code });
        } else {
          // multiple product matches, no single exact → genuinely ambiguous → prompt
          stillAmbiguous.push(r);
        }
      } else {
        // NON-PRODUCT (attachment_type, etc.): no variant problem.
        if (matches.length === 1) {
          const m = matches[0];
          exact_entities.push({ uuid: m.uuid, entity_type: m.entity_type, code: m.canonical_code });
        } else {
          // multiple non-product matches → still ambiguous, let user pick
          stillAmbiguous.push(r);
        }
      }
    }
    if (stillAmbiguous.length > 0) {
      specific_options = stillAmbiguous.map(r => ({
        token: r.token,
        candidates: (r.matches ?? []).filter(isCompatible).map(m => ({
          uuid: m.uuid,
    ...
    const exacts = andMatches.filter(m => m && isCompatible(m) && typedTokens.has(norm(m.canonical_code)));
    if (exacts.length === 1) {
      const m = exacts[0];
      exact_entities.push({ uuid: m.uuid, entity_type: m.entity_type, code: m.canonical_code });
    } else {
      // no single exact → all compatible matches are candidates to choose from
      const candidates = andMatches.filter(isCompatible);
