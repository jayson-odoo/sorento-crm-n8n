...
| tier-ask lane build clone (tier-gate/tier-probe*/if-tier-* are NOT on `txiPz…`) | fork `RnpxEnAV3g20MmKj` (PROMO-PICKER), driver `zz-canary-run PROMO-PICKER M5m6EYDLdSc0ofto` = `POST /webhook/zz-run-promo-picker {test_run_id,contact,item}`; see `tests/runs/promotion-hotfix-20260818.md` |
...
- No native n8n MCP tools in the session? The MCP server is plain streamable-HTTP JSON-RPC — `curl -X POST $N8N_MCP_URL -H "Authorization: Bearer <.mcp.json token>" -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"<tool>","arguments":{…}}}'` (SSE reply, take the `data:` line). `test_workflow` with a pinned `When Executed by Another Workflow` on `sub-get-results TEST rysSPgUssLDf6xJc` = arbitrary read-only CRM MCP probe (MCP Client1 runs real).
