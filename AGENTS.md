...
## No n8n MCP in the session? REST fallback (proven 2026-08-17)

If `mcp__n8n-mcp__*` tools are absent (no `.mcp.json` at session start), the full build/test loop works over the public REST API + webhooks — procedure, settings-whitelist PUT body, `POST /workflows/{id}/activate` (= publish), and the chat-console UAC curl loop (`/webhook/zz-chat-console` seed → `/webhook/zz-dispatch-test` fire → `/webhook/zz-chat-read` reply → `GET /executions/{id}?includeData=true`) are documented in `n8n-workflows-init/tests/diffs/mc-label-n8n.md` and `n8n-workflows-init/tickets/mc-label-n8n-handoff.md`. PUT updates the DRAFT only and preserves pinData + the settings keys its schema rejects; activate publishes and runs node-config validation.
